...
<?php
	...
	if(isset($_GET['view_id']) && !empty($_GET['view_id']))
	{
		...
		$stmt_edit = $DB_con->prepare('SELECT * FROM users WHERE user_id=:user_id'); //Select de la bd
...
<body>
  ...
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">Detalles del Pedido del Cliente</li>
  </ol>
</nav>
        ...
        //Conexión con la bd
        include("config.php");
        	$stmt = $DB_con->prepare("SELECT * FROM orderdetails where user_id='$user_id' and order_status='Ordered'"); //Select de la bd
        	$stmt->execute();
        	//Condición If
        	if($stmt->rowCount() > 0)
        	{  //Bucle While
		...
		//Conexión con la bd
		include("config.php");
		 $stmt_edit = $DB_con->prepare("select sum(order_total) as totalx from orderdetails where user_id=:user_id and order_status='Ordered'"); //Sumatorio y recogida de datos
		...
		echo "<tr>";
		echo "<td colspan='3' align='right' style='font-size:20px;'>Cliente: ".$user_firstname." ".$user_lastname." | <span style='color:red'>Precio Total:</span>";
		echo "</td>";
